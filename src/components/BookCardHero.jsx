	...
	return (
		<div
			...
			<div className="book-card-hero__cover-wrap">
				{record.coverUrl ? (
					<img
						className="book-card-hero__cover"
						src={record.coverUrl}
						alt={record.book}
						onError={(e) => {
				...
				<div
					className="book-card-hero__cover-placeholder"
			...
			<div className="book-card-hero__body">
				<div className="book-card-hero__top">
					<span className="book-card-hero__no">
						No.{record.no}
						{record.emoji && <span className="book-card-hero__emoji">{record.emoji}</span>}
					</span>
				</div>
				<h3 className="book-card-hero__title">{record.book}</h3>
				{record.author && <p className="book-card-hero__author">{record.author}</p>}
				{record.chapter && <div className="book-card-hero__chapter">📖 {record.chapter}</div>}
				{mvp && (
					<div className="book-card-hero__mvp">
						{mvp.keys.map((k) => {
							...
							return (
								<span
									key={k}
									className="book-card-hero__mvp-badge"
				...
				{/* 화두 목록 (topics 배열) */}
				{record.topics?.filter((t) => t).length > 0 && (
					<div className="book-card-hero__topics">
						<span className="book-card-hero__topic-label">💬 화두</span>
						{record.topics
							.filter((t) => t)
							.map((t, i) => (
								<div key={i} className="book-card-hero__topic-item">
									<span className="book-card-hero__topic-num">{i + 1}</span>
									<span>{t}</span>
								</div>
							))}
					</div>
				...
				<div className="book-card-hero__footer">
					{record.meetingType && <span className="book-card-hero__tag">{record.meetingType}</span>}
					{record.presenter && <span className="book-card-hero__tag">발제 {record.presenter}</span>}
					{record.participants?.length > 0 && (
						<span className="book-card-hero__tag">{record.participants.join(" · ")}</span>
					)}
